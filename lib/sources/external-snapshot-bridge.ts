...
export function snapshotBaseUrl(): string | null {
...
export function externalSnapshotBridgeUrl(period: MatchIntelPeriod, date: Date): string | null {
  const base = snapshotBaseUrl();
  if (!base) return null;
  return `${base}/stats_${period}_${ymd(date)}.json?v=3`;
}

export async function fetchExternalSnapshotBridgeForDate(
  period: MatchIntelPeriod,
  date: Date,
  options?: { revalidateSeconds?: number; throwOnError?: boolean }
): Promise<{ snapshot: MatchIntelSnapshot } | null> {
  const url = externalSnapshotBridgeUrl(period, date);
  if (!url) return null;
  ...
  try {
    ...
    if (!snapshot.leaders.length) return null;
...
export async function fetchExternalSnapshotBridge(
  ...
  for (let i = 0; i <= maxLookback; i++) {
    ...
    const exact = await fetchExternalSnapshotBridgeForDate(period, d);
    if (exact?.snapshot?.leaders?.length) return exact;
  }
