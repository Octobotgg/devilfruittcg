...
function trendSymbol(trend: MatchupDeck["trend"]): MetaDeck["trend"] {
  if (trend === "up") return "▲";
  if (trend === "down") return "▼";
  return "—";
}

export function matchupDecksToMetaSnapshot(
  decks: MatchupDeck[],
  options: {
    source: string;
    updatedAt: string;
  ...
  return {
    ...
    sampleGames: options.sampleGames,
    sampleLabel: options.sampleLabel,
    sampleDescription: options.sampleDescription,
    comparableSample: options.comparableSample,
    metaDecks: decks.slice(0, 15).map((deck, index) => ({
      rank: index + 1,
      name: deck.name,
      tier: deck.tier === "D" ? "C" : deck.tier,
      color: deck.color,
      winRate: deck.winRate ?? null,
      popularity: deck.metaShare,
      trend: trendSymbol(deck.trend),
      cardId: deck.cardId,
    })),
    regions: options.regions || [{ region: "Global", events: 0, players: 0 }],
    decks: SEEDED_DECKS,
...
const SEEDED_META_UPDATED_AT = "2024-01-01T00:00:00.000Z";
...
export function getSeededMeta(): MetaSnapshot {
  ...
  return {
    updatedAt: SEEDED_META_UPDATED_AT,
    source: "seeded",
    metaDecks,
    regions,
    decks: SEEDED_DECKS,
