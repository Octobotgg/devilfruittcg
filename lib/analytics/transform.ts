...
function rowDisplayName(row: LeaderDailyStatRow): string {
  const card = getOfficialCardById(row.leader_id);
...
function buildMatchupLookup(rows: LeaderMatchupDailyStatRow[]): Map<string, Map<string, number>> {
  ...
  const ensureRow = (leaderId: string) => {
    const existing = lookup.get(leaderId);
    if (existing) return existing;
    const created = new Map<string, number>();
    lookup.set(leaderId, created);
    return created;
  };

  for (const row of rows) {
    const map = ensureRow(row.leader_id);
