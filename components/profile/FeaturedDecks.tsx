...
function DeckCard({ deck }: { deck: ProfileFeaturedDeck }) {
  const content = (
    <div className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.03]">
      <div className="relative h-44 overflow-hidden bg-black/30 sm:h-48">
        {deck.leaderImageId ? (
          <img
          ...
          <div className="flex h-full items-center justify-center text-sm font-bold text-white/35">No leader art</div>
        )}
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/55 to-transparent px-4 pb-4 pt-10">
          ...
          <p className="text-sm text-white/60">{deck.leaderName || "No leader set"}</p>
        </div>
      </div>
      ...
      <div className="space-y-4 p-4">
        <div className="flex flex-wrap gap-2">
          {deck.leaderColors.length ? deck.leaderColors.map((color) => (
            <span
              ...
              className="inline-flex items-center gap-1 rounded-full border border-white/12 bg-black/30 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-white/75"
            >
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: COLOR_HEX[color] || "#999" }} />
              {color}
            </span>
          )) : (
            <span className="inline-flex items-center gap-1 rounded-full border border-white/12 bg-black/30 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-white/50">
              Unknown color
            </span>
          )}
        </div>
        ...
        <div className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/12 bg-black/25 px-4 text-sm font-bold text-white/80 transition-colors hover:text-white">
          View decklist
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-black/20 p-6">
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.14em] text-white/40">Deck Showcase</p>
          <h2 className="mt-2 text-2xl font-black text-white">Flagship Decks</h2>
        </div>
      ...
      {decks.length ? (
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          {Array.from({ length: emptySlots }).map((_, index) => (
            <Link
              ...
              href="/decks"
              className="flex min-h-[18rem] flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-white/15 bg-white/[0.02] p-6 text-center transition-colors hover:border-white/25 sm:min-h-[21rem]"
            >
              ...
              <p className="mt-4 text-lg font-black text-white">Pin a deck</p>
              <p className="mt-2 max-w-[16rem] text-sm text-white/55">
        ...
        <p className="mt-5 text-sm text-white/45">No featured decks yet.</p>
      )}
    </section>
  );
}
