...
export type MatchupsInitialPayload = {
  source: string;
...
export default function MatchupsPageClient({
  ...
  const [sourceLabel, setSourceLabel] = useState<string>(initialIsLive ? "Live aggregate" : "Live data unavailable");
  const [sampleGames, setSampleGames] = useState<number>(initialIsLive ? initialPayload.sampleGames : 0);
  const [sampleLabel, setSampleLabel] = useState<string | null>(initialIsLive ? initialPayload.sampleLabel : "Live data unavailable");
  const [sampleDescription, setSampleDescription] = useState<string | null>(
    ...
    const run = async () => {
      try {
        ...
        if (Array.isArray(json.decks) && json.decks.length) {
          ...
          setSourceLabel(seeded ? "Live data unavailable" : "Live aggregate");
          if (!seeded && typeof json.sampleGames === "number") setSampleGames(json.sampleGames);
          else setSampleGames(0);
          setSampleLabel(seeded ? "Live data unavailable" : typeof json.sampleLabel === "string" ? json.sampleLabel : null);
          setSampleDescription(seeded ? "Showing fallback matchup snapshot" : typeof json.sampleDescription === "string" ? json.sampleDescription : null);
          setComparableSample(seeded ? false : json.comparableSample !== false);
          ...
          >
            ...
            <div className="relative grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-[var(--color-parchment-dark)] bg-[var(--color-parchment)] p-4">
                ...
                <p className="mt-2 text-lg font-black text-[var(--color-navy)]">{topDeck?.name ?? "—"}</p>
                <p className="text-sm text-[#F0C040]">{topDeck?.metaShare ?? 0}% meta share</p>
              </div>
              <div className="rounded-2xl border border-[var(--color-parchment-dark)] bg-[var(--color-parchment)] p-4">
                <p className="text-[11px] tracking-[0.18em] uppercase text-[var(--color-text-light)]">Hardest punish</p>
