...
import DeckViewModal from "@/components/decks/DeckViewModal";
...
import { buildDeckAnalytics } from "@/lib/decks/deck-modal-analytics";
import { buildDeckCollectionCoverage } from "@/lib/decks/deck-collection-coverage";
import { buildDeckMatchupSnapshot } from "@/lib/decks/deck-modal-matchups";
import { buildDeckSimExportLines } from "@/lib/decks/deck-sim-export";
...
export default function DecksPageClient({ initialMatchupDecks = [] }: DecksPageClientProps) {
  const {
    loadDecks: loadDecksFromStore,
    saveDecks: saveDecksToStore,
  ...
  }, [cloudReady, loadCollectionFromStore]);
  ...
  const expandedDeckLeader = useMemo(
    () => (expandedDeck?.leaderId ? cardCache.get(expandedDeck.leaderId) || null : null),
    [cardCache, expandedDeck],
  );
  const expandedDeckPreviewGroups = useMemo(
    () => (expandedDeck ? buildDeckPreviewGroups(expandedDeck, cardCache) : []),
    [cardCache, expandedDeck],
  );
  const expandedDeckCoverage = useMemo(
    () => (expandedDeck ? buildDeckCollectionCoverage(expandedDeck, collection, cardCache) : null),
    [cardCache, collection, expandedDeck],
  );
  const expandedDeckAnalytics = useMemo(() => {
    if (!expandedDeck) return null;

    return buildDeckAnalytics(
      expandedDeck.cards
        .map((entry) => {
          const card = cardCache.get(entry.cardId);
          return card ? { card, quantity: entry.quantity } : null;
        })
        .filter((entry): entry is { card: Card; quantity: number } => Boolean(entry)),
    );
  }, [cardCache, expandedDeck]);
  const expandedDeckMatchupRows = useMemo(
    () => buildDeckMatchupSnapshot(expandedDeck?.leaderId, initialMatchupDecks, 6),
    [expandedDeck?.leaderId, initialMatchupDecks],
  );
  const expandedDeckSimExport = useMemo(
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <DeckViewModal
        open={Boolean(expandedDeck)}
        deck={expandedDeck}
        leader={expandedDeckLeader}
        totalCards={expandedDeck ? mainDeckCardCount(expandedDeck) : 0}
        deckValueLabel={
          expandedDeckPricesLoading && expandedDeckPrices.size === 0
            ? "..."
            : formatCurrency(expandedDeckPriceSummary.total)
        }
        groups={expandedDeckPreviewGroups}
        priceQuotes={expandedDeckPrices}
        collectionCoverage={expandedDeckCoverage}
                      ...
                      <div className="relative space-y-3 p-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[var(--color-parchment-dark)] bg-[var(--color-parchment)] px-3 py-2 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--color-text-dark)] transition-colors hover:border-[var(--color-gold)] hover:text-[var(--color-gold-dark)]"
                            title="View deck"
                          >
                            <Eye className="h-3.5 w-3.5" />
                            <span className="hidden sm:inline">View Deck</span>
                          </button>
