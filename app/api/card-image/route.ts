...
export async function GET(req: NextRequest) {
  ...
  const fallbackUrl = `https://en.onepiece-cardgame.com/images/cardlist/card/${encodeURIComponent(cardId)}.png`;
  const candidateUrls = Array.from(new Set([card?.imageUrl, fallbackUrl].filter(Boolean))) as string[];
  ...
  try {
    for (const targetUrl of candidateUrls) {
      const res = await fetch(targetUrl, {
        headers: {
          ...
          Origin: "https://en.onepiece-cardgame.com",
        },
      });
    ...
    // fall through to redirect/placeholder fallback
  }

  if (candidateUrls.length) {
    return NextResponse.redirect(candidateUrls[0], {
      status: 307,
